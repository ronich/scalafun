 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("one plus one is three?") {
    assert(1 + 1 == 2
    ) // This assertion fails! Go ahead and fix it.
  }
  ...
  test("sum of lists with repeated elements") {
    assert(sum(List(1, 1, 1, 1)) === 4)
  }
  test("sum of list of zeroes") {
  ...
  test("max of lists with repeated elements") {
    assert(max(List(1, 1, 1, 1)) === 1)
  }
  test("max of list of zeroes") {
