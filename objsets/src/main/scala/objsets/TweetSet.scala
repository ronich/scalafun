...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new NoSuchElementException("Empty TweetSet has no most retweeted Tweet")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    if (!left.isEmpty && elem.retweets < leftMost.retweets) {
      if (!right.isEmpty && leftMost.retweets < rightMost.retweets)
        rightMost
      else
        leftMost
    } else {
      if (!right.isEmpty && elem.retweets < rightMost.retweets)
        rightMost
      else
        elem
    }
